...
public class BlockCopperPile extends Block {
    ...
    public BlockCopperPile() {
        super(Material.GROUND);
        this.setDefaultState(this.blockState.getBaseState().withProperty(LAYERS, 1));
        this.setTickRandomly(true);
        this.setCreativeTab(IceAndFire.TAB_BLOCKS);
        this.setTranslationKey("iceandfire.copperpile");
        this.setHardness(0.3F);
        this.setSoundType(IafBlockRegistry.SOUND_TYPE_GOLD);
        setRegistryName(IceAndFire.MODID, "copperpile");
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (!item.isEmpty()) {
            if (item.getItem() != null) {
                if (item.getItem() == Item.getItemFromBlock(IafBlockRegistry.copperPile)) {
                    if (!item.isEmpty()) {
                        if (this.getMetaFromState(state) < 7) {
                            WorldUtils.setBlock(worldIn, pos.getX(), pos.getY(), pos.getZ(), IafBlockRegistry.copperPile, this.getMetaFromState(state) + 1, 3);
    ...
    public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return IafItemRegistry.copperNugget;
    }
