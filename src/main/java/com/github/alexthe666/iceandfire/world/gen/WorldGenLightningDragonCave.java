...
public class WorldGenLightningDragonCave extends WorldGenerator {
    ...
    private static final WorldGenCaveStalactites CEILING_DECO = new WorldGenCaveStalactites(IafBlockRegistry.crackledStone);
    ...
    public static void setGoldPile(World world, BlockPos pos, Random rand) {
        ...
        if (!(world.getBlockState(pos).getBlock() instanceof BlockContainer)) {
            if (chance < 60) {
                ...
                world.setBlockState(pos, generateGold ? IafBlockRegistry.copperPile.getDefaultState().withProperty(BlockCopperPile.LAYERS, 1 + rand.nextInt(7)) : Blocks.AIR.getDefaultState(), 3);
            } else if (chance == 61) {
                ...
                if (world.getBlockState(pos).getBlock() instanceof BlockChest) {
                    ...
                    if (tileentity1 instanceof TileEntityChest && !tileentity1.isInvalid()) {
                        ((TileEntityChest) tileentity1).setLootTable(isMale ? LIGHTNINGDRAGON_MALE_CHEST : LIGHTNINGDRAGON_CHEST, rand.nextLong());
                    }
                }
            }
        }
                ...
                if (!(worldIn.getBlockState(position).getBlock() instanceof BlockContainer) && worldIn.getBlockState(position).getBlockHardness(worldIn, position) >= 0) {
                    ...
                    if (doOres) {
                        ...
                        if (chance > 90 && chance < 1000) {
                            worldIn.setBlockState(blockpos, IceAndFire.CONFIG.generateAmythestOre ? IafBlockRegistry.amythestOre.getDefaultState() : Blocks.EMERALD_ORE.getDefaultState(), 3);
                        }
                    } else {
                        worldIn.setBlockState(blockpos, rand.nextBoolean() ? IafBlockRegistry.crackledCobblestone.getDefaultState() : IafBlockRegistry.crackledStone.getDefaultState());
                    }
                }
            }
        }
