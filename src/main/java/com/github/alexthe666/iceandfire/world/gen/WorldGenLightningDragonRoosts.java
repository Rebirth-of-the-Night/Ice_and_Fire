...
public class WorldGenLightningDragonRoosts extends WorldGenerator {
        ...
        int radius = 12 + rand.nextInt(8);
        {
            ...
            for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, k, -l), position.add(j, 0, l))) {
                ...
                if (blockpos.distanceSq(position) <= (double) (f * f) && yAdd < 2 + rand.nextInt(k) && !worldIn.isAirBlock(blockpos.down())) {
                    worldIn.setBlockState(blockpos, IafBlockRegistry.crackledGrass.getDefaultState());
                }
            }
            for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, k, -l), position.add(j, 0, l))) {
                if (worldIn.getBlockState(blockpos).getBlock() == IafBlockRegistry.crackledGrass && !worldIn.isAirBlock(blockpos.up())) {
                    worldIn.setBlockState(blockpos, IafBlockRegistry.crackledDirt.getDefaultState());
                }
            }
        }
        {
            ...
            for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, -k, -l), position.add(j, 0, l))) {
                if (blockpos.distanceSq(position) < (double) (f * f)) {
                    worldIn.setBlockState(blockpos, rand.nextBoolean() ? IafBlockRegistry.crackledGravel.getDefaultState() : IafBlockRegistry.crackledDirt.getDefaultState());
                }
                if (blockpos.distanceSq(position) == (double) (f * f)) {
                    worldIn.setBlockState(blockpos, rand.nextBoolean() ? IafBlockRegistry.crackledCobblestone.getDefaultState() : IafBlockRegistry.crackledCobblestone.getDefaultState());
                }
            }
        }
        radius -= 2;
            ...
            for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, -k, -l), position.add(j, k, l))) {
                if (blockpos.distanceSq(position) <= (double) (f * f)) {
                    ...
                    if (dist > 0.5D && rand.nextInt(1000) == 0) {
                        ...
                        new WorldGenRoostBoulder(IafBlockRegistry.crackledCobblestone, rand.nextInt(3), true).generate(worldIn, rand, height);
                    }
                    ...
                    if (dist < 0.3D && rand.nextInt(isMale ? 250 : 400) == 0) {
                        ...
                        new WorldGenRoostGoldPile(IafBlockRegistry.copperPile).generate(worldIn, rand, height);
                    }
                    if (dist < 0.3D && rand.nextInt(isMale ? 500 : 700) == 0) {
                        ...
                        if (worldIn.getBlockState(height).getBlock() instanceof BlockChest) {
                            ...
                            if (tileentity1 instanceof TileEntityChest && !tileentity1.isInvalid()) {
                                ((TileEntityChest) tileentity1).setLootTable(WorldGenLightningDragonCave.LIGHTNINGDRAGON_CHEST, new Random().nextLong());
                            }
                        }
                    }
                    if (rand.nextInt(5000) == 0) {
                        BlockPos height = worldIn.getHeight(blockpos);
                        new WorldGenRoostArch(IafBlockRegistry.crackledCobblestone).generate(worldIn, rand, height);
                    }
                }
            }
        }
