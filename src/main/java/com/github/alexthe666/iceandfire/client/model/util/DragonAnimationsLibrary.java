...
public class DragonAnimationsLibrary {
    ...
    public static IceAndFireTabulaModel getModel(IEnumDragonPoses pose, IEnumDragonModelTypes modelType) {
    	IceAndFireTabulaModel result = models.get(toKey(pose, modelType));
        if(result == null)
    ...
    public static void registerReference(IEnumDragonPoses pose, IEnumDragonModelTypes modelSource, IEnumDragonModelTypes modelDestination) {
    	IceAndFireTabulaModel source = getModel(pose, modelSource);
