...
public class IceDragonTabulaModelAnimator extends IceAndFireTabulaModelAnimator implements IIceAndFireTabulaModelAnimator<EntityIceDragon> {
    ...
    private final List<IceAndFireTabulaModel<? extends EntityDragonBase>> swimPoses = Arrays.asList(
    ...
    public void setRotationAngles(IceAndFireTabulaModel<EntityIceDragon> model, EntityIceDragon entity, float limbSwing, float limbSwingAmount, float ageInTicks, float rotationYaw, float rotationPitch, float scale) {
        ...
        IceAndFireTabulaModel<? extends EntityDragonBase> currentPosition = swimming ? swimPoses.get(currentIndex) : walking ? walkPoses.get(currentIndex) : flyPoses.get(currentIndex);
        IceAndFireTabulaModel<? extends EntityDragonBase> prevPosition = swimming ? swimPoses.get(prevIndex) : walking ? walkPoses.get(prevIndex) : flyPoses.get(prevIndex);
